...
1. A user invokes a new agent via `@agent-name` or `/AEXOS:agents:agent-name`
...
Handoff artifacts are stored at `.aexos/handoffs/` (runtime, gitignored). Format: `handoff-{from}-to-{to}-{timestamp}.yaml`.

## Template Reference

Full template: `.aexos-core/development/templates/agent-handoff-tmpl.yaml`
