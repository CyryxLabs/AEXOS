...
REST API用于将squads同步到AEXOS并发现marketplace中的squads。

## 概述

Squads API允许：

- **同步**: 将本地squads发送到AEXOS云端
- **Marketplace**: 发现和浏览公共squads
- **管理**: 更新可见性、删除squads

**基础URL**: `https://api.cyryx.ai`
...
获取您的API密钥：https://cyryx.ai/settings/api-keys
...
```json
{
  "info": {
    ...
    "description": "AEXOS Squads Marketplace的REST API",
...
将上述JSON保存为 `aexos-squads-api.postman_collection.json` 并导入Postman。
