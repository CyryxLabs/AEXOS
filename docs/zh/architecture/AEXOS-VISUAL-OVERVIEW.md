...
flowchart TB
    subgraph COLLAB["小队 + 人类协作"]
        ...
        subgraph PLANNING["阶段 1: 规划"]
            H_PM["人类 PM"] <-->|"完善需求"| AI_PM["@pm (Janus)<br/>代理"]
            H_PM <-->|"验证架构"| AI_ARCH["@architect (Vega)<br/>代理"]
            AI_PM --> PRD["完整 PRD"]
            AI_ARCH --> ARCH_DOC["架构<br/>文档"]
        end

        subgraph EXECUTION["阶段 2: 开发"]
            AI_SM["@sm (Chronos)<br/>代理"] -->|"创建故事"| STORIES["超详细<br/>故事"]
            STORIES --> AI_DEV["@dev (Vulcan)<br/>代理"]
            AI_DEV -->|"实现"| CODE["代码"]
            CODE --> AI_QA["@qa (Argus)<br/>代理"]
            H_DEV["人类开发者"] <-->|"结对编程"| AI_DEV
            H_DEV <-->|"代码审查"| AI_QA
        end

        subgraph REVIEW["阶段 3: 质量门禁"]
            AI_QA -->|"通过"| AI_DEVOPS["@devops (Polaris)<br/>代理"]
            AI_QA -->|"失败"| AI_DEV
...
flowchart TD
    ...
    ANALYST --> PM["@pm (Janus)<br/>创建 PRD"]
    PM --> ARCH["@architect (Vega)<br/>架构设计"]

    ARCH --> SM["@sm (Chronos)<br/>创建故事"]
    SM --> PO{"@po (Themis)<br/>验证故事?<br/>10点检查表"}

    PO -->|"通过 >= 7/10"| DEV["@dev (Vulcan)<br/>实现"]
    PO -->|"不通过 < 7/10"| SM

    DEV --> CODERABBIT{"CodeRabbit<br/>自愈<br/>最多 2 次迭代"}
    CODERABBIT -->|"严重"| DEV
    CODERABBIT -->|"通过"| QA{"@qa (Argus)<br/>质量门禁<br/>7 项检查"}

    QA -->|"通过"| DEVOPS["@devops (Polaris)<br/>推送 + PR"]
    QA -->|"顾虑"| DEVOPS
    QA -->|"失败"| DEV
