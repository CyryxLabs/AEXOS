...
graph LR
    subgraph "规划代理"
        Architect["@architect<br/>Vega<br/>整体系统架构师"]
        PM["@pm<br/>Janus<br/>产品经理"]
        Analyst["@analyst<br/>Sirius<br/>业务分析师"]
    end

    subgraph "管理代理"
        PO["@po<br/>Themis<br/>产品所有者"]
        SM["@sm<br/>Chronos<br/>敏捷主管"]
    end

    subgraph "执行代理"
        Dev["@dev<br/>Vulcan<br/>全栈开发人员"]
        QA["@qa<br/>Argus<br/>测试架构师"]
    end
...
| 代理 | 名称 | 角色 | 工作流中的职责 |
|------|------|------|---------------|
| @architect | Vega | 整体系统架构师 | 现有服务分析、架构创建 |
| @pm | Janus | 产品经理 | 为棕地项目创建PRD |
| @po | Themis | 产品所有者 | 工件验证、文档分片、回顾 |
| @sm | Chronos | 敏捷主管 | 故事创建 |
| @analyst | Sirius | 业务分析师 | 故事草稿的可选审查 |
| @dev | Vulcan | 全栈开发人员 | 故事实现、修复反馈 |
| @qa | Argus | 测试架构师 | 实现审查、质量关卡 |
...
| 问题 | 首次联系 | 升级到 |
|------|--------|--------|
| PRD不完整 | @pm (Janus) | @po (Themis) |
| 架构不一致 | @architect (Vega) | @pm (Janus) |
| 故事模糊 | @sm (Chronos) | @po (Themis) |
| 实现有错误 | @dev (Vulcan) | @qa (Argus) |
| 质量关卡失败 | @qa (Argus) | @po (Themis) |
| 集成中断 | @architect (Vega) | @devops (Polaris) |
