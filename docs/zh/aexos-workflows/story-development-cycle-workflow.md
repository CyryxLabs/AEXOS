...
Scrum Master (Chronos) 使用 PRD 共享或项目文档作为来源识别和创建下一个故事。
...
产品所有者 (Themis) 使用严格的 10 点检查清单验证已创建的故事，确保故事已准备好实现。
...
Dev Agent (Vulcan) 实现已验证的故事，遵循验收标准和定义的任务。包括 CodeRabbit 自我修复循环以确保代码质量。
...
QA Agent (Argus) 执行最终审查和质量网关，验证代码、测试和验收标准的遵循。
