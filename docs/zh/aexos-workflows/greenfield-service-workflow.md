...
sequenceDiagram
    autonumber
    participant U as 用户
    participant AN as 分析师 (Sirius)
    participant PM as PM (Janus)
    participant AR as 架构师 (Vega)
...
| ID | 名称 | 标题 | 图标 | 工作流中的责任 |
|----|------|------|------|----------------|
| `analyst` | Sirius | 业务分析师 | `分析` | 创建项目简报、头脑风暴、市场研究 |
| `pm` | Janus | 产品经理 | `战略` | 创建和更新 PRD |
| `architect` | Vega | 架构师 | `架构` | 创建服务架构 |
| `po` | Themis | 产品所有者 | `验证` | 验证工件、分片文档、回顾 |
| `sm` | Chronos | Scrum 大师 | `便利` | 创建史诗的故事 |
| `dev` | Vulcan | 全栈开发人员 | `实现` | 实施故事 |
| `qa` | Argus | 测试架构师 | `质量` | 审阅实现 |
...
| 文件 | 代理 |
|------|------|
| `.aexos-core/development/agents/analyst.md` | Sirius (分析师) |
| `.aexos-core/development/agents/pm.md` | Janus (PM) |
| `.aexos-core/development/agents/architect.md` | Vega (架构师) |
