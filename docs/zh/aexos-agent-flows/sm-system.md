...
**@sm (Chronos)** 代理是 AEXOS 的技术 Scrum Master，专门负责故事准备和敏捷流程促进。其主要角色是创建详细、可操作的故事，使开发代理能够以最少的额外研究进行实现。
...
flowchart TB
    ...
    subgraph COLLABORATION["协作"]
        DEV_AGENT["@dev (Vulcan)<br/>接收故事"]
        PO_AGENT["@po (Themis)<br/>验证故事"]
        DEVOPS_AGENT["@github-devops (Polaris)<br/>完成后 Push/PR"]
    end
...
flowchart TB
    subgraph UPSTREAM["上游 - 提供给 @sm"]
        PM_UP["@pm (Janus)<br/>创建史诗结构"]
        PO_UP["@po (Themis)<br/>优先排序待办"]
    ...
    subgraph DOWNSTREAM["下游 - 从 @sm 接收"]
        DEV_DOWN["@dev (Vulcan)<br/>实现故事"]
        PO_DOWN["@po (Themis)<br/>验证故事"]
        QA_DOWN["@qa (Argus)<br/>风险分析"]
    end

    subgraph LATERAL["横向 - 与 @sm 协调"]
        DEVOPS_LAT["@github-devops (Polaris)<br/>Push/PR 工作流"]
    end
...
| 代理 | 关系 | 操作 |
|--------|----------------|------|
| **@pm (Janus)** | 接收自 | 史诗结构、已分片 PRD |
| **@po (Themis)** | 协调 | 待办优先排序、冲刺规划 |
| **@dev (Vulcan)** | 交付给 | 准备实现的故事 |
| **@qa (Argus)** | 请求 | 故事的风险分析 |
| **@github-devops (Polaris)** | 委派给 | 推送分支、创建 PR |
...
2. 或执行 AEXOS 安装器：`npm run cyryx:install`
