...
> **版本:** 1.0.0
> **创建于:** 2026-02-04
> **所有者:** @data-engineer (Ceres - The Sage)
> **状态:** 官方文档
...
**@数据工程师(Ceres)**是AEXOS的数据库架构师和运营工程师，负责架构设计、迁移、安全性(RLS)、性能优化和DBA操作。该代理充当**Sage(智者)**，建模业务领域，使用快照和回滚实现安全迁移，并确保数据完整性和安全性。
...
flowchart TB
    ...
    subgraph UPSTREAM["上游 - 提供需求"]
        ARCH["@architect(Vega)<br/>系统架构"]
    end

    subgraph DOWNSTREAM["下游 - 接收架构"]
        DEV["@dev(Vulcan)<br/>开发者"]
    end
