...
**小队创建者** (Arkantos) 是 AEXOS 用于创建、验证、发布和管理小队的专业代理。小队是代理、任务、工作流和资源的模块化包，可在项目之间重用。
...
- **列出小队**项目本地
- **分发小队**在 3 个级别（本地、aexos-squads、AEXOS API）
- **迁移小队**到带编排和技能的 v2 格式
- **分析和扩展**现有小队
...
4. **3 级分发**：本地、公共（aexos-squads）、市场（AEXOS API）
...
| `.aexos-core/development/tasks/squad-creator-sync-aexos.md` | `*sync-squad-cyryx` | 同步小队到 AEXOS API | 占位符（Sprint 8） |
...
flowchart TB
    ...
    subgraph DISTRIBUTE["分发"]
        LOCAL["本地<br/>./squads/"]
        PUBLIC["公共<br/>github.com/CyryxLabs/aexos-squads"]
        MARKET["市场<br/>api.cyryx.dev/squads"]
    end
...
| `*sync-squad-cyryx` | `squad-creator-sync-aexos.md` | 同步小队到 AEXOS API |
...
```mermaid
flowchart LR
    subgraph SQUAD_CREATOR["@squad-creator (Arkantos)"]
    ...
    subgraph DEVOPS["@devops (Polaris)"]
        DEVOPS_PUB["发布"]
        DEVOPS_DEPLOY["部署"]
    end
