...
> **架构:** MIS 遵循 AEXOS 的 **Open Core** 模型。所有记忆智能都位于私有仓库 `aexos-pro`（子模块 `pro/`）中。`aexos-core`（开源）仅提供扩展点，并且在没有智能记忆的情况下 100% 正常运行 — 与现在完全一样。
...
| **捕获** | 通过 hooks 捕获会话知识 | **aexos-pro** | MIS-3 |
| **存储** | 使用带 YAML frontmatter 的 Markdown 存储 | **aexos-pro** | MIS-3, MIS-4 |
| **检索** | 按相关性渐进式检索 | **aexos-pro** + **aexos-core** 扩展点 | MIS-4, MIS-6 |
| **演化** | 自动学习和演化规则 | **aexos-pro** | MIS-5, MIS-7 |

### 关键原则 (vs 当前状态)

| 方面 | 当前状态 | 使用 MIS (aexos-pro 激活) | 没有 MIS (仅 aexos-core) |
|---------|-------------|--------------------------|--------------------------|
...
> **aexos-core 在没有 MIS 的情况下 100% 正常运行** — 与现在的运行方式完全相同。当 `aexos-pro` 存在时（子模块 `pro/`），MIS 通过扩展点自动连接。无需手动配置。
...
flowchart LR
    ...
    subgraph PRO["aexos-pro (私有, 子模块 pro/)"]
        direction TB
...
flowchart TB
    ...
    subgraph PRO_BOUNDARY["aexos-pro (私有 — pro/)"]
...
> **仓库:** `aexos-pro` (pro/memory/memory-retriever.js, memory-loader.js)
...
> **仓库:** `aexos-pro` (pro/memory/self-learner.js, rule-proposer.js)
...
> **仓库:** `aexos-pro` (pro/memory/agent-memory-api.js)
...
> **100% 在 aexos-pro 中** — core 中没有自学习组件
...
| 方面 | 当前 aexos-core | aexos-core + aexos-pro (MIS) | 没有 pro 的 aexos-core |
|---------|---------------|---------------------------|------------------|
...
| MIS-3 | 会话摘要 (PreCompact Hook) | **aexos-core** (hook runner) + **aexos-pro** (digest) | MIS-1 | 14h |
| MIS-4 | 渐进式记忆检索 | **aexos-pro** | MIS-3 | 16h |
| MIS-5 | 自学习引擎 | **aexos-pro** | MIS-3, MIS-4 | 14h |
| MIS-6 | 管道集成与代理记忆 API | **aexos-core** (扩展点) + **aexos-pro** (loader) | MIS-4 | 10h |
| MIS-7 | CLAUDE.md 与规则自动演化 | **aexos-pro** | MIS-5 | 8h |
| **总计** | | | | **~78h** |
...
| Epic PRO 架构 | [EPIC-PRO-INDEX.md](../stories/epics/epic-pro-aexos-pro-architecture/EPIC-PRO-INDEX.md) |
