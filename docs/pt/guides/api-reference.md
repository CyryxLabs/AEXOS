...
| ---------------- | ------ | ------------ | ----------------------------------------------- |
| `@dev`           | Vulcan    | Builder      | Implementação de código, debugging, testes      |
| `@qa`            | Argus  | Guardian     | Garantia de qualidade, code review, testes      |
| `@architect`     | Vega   | Visionary    | Arquitetura de sistema, design de API           |
| `@pm`            | Janus | Strategist   | Requisitos de produto, epics, estratégia        |
| `@po`            | Themis    | Champion     | Gestão de backlog, critérios de aceitação       |
| `@sm`            | Chronos  | Facilitator  | Planejamento de sprint, criação de stories      |
| `@analyst`       | Sirius  | Explorer     | Pesquisa de mercado, análise competitiva        |
| `@data-engineer` | Ceres   | Architect    | Schema de banco, migrations, queries            |
| `@devops`        | Polaris   | Optimizer    | CI/CD, deployment, operações git                |
| `@ux-expert`     | Uma    | Creator      | Design UI/UX, wireframes                        |
| `@aexos-master`   | Zeus  | Orchestrator | Orquestração do framework, meta-operações       |
...
_AEXOS (Cyryx) Referência de API v4.2.11_
