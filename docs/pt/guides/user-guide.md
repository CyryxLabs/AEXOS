...
| --------- | ---------------- | ------------ | ------------------------- |
| **Vulcan**   | `@dev`           | Construtor   | Implementação de código   |
| **Argus** | `@qa`            | Guardião     | Garantia de qualidade     |
| **Vega**  | `@architect`     | Arquiteto    | Arquitetura técnica       |
| **Nova**  | `@po`            | Visionário   | Backlog do produto        |
| **Kai**   | `@pm`            | Equilibrador | Estratégia do produto     |
| **Chronos** | `@sm`            | Facilitador  | Facilitação de processos  |
| **Zara**  | `@analyst`       | Explorador   | Análise de negócios       |
| **Ceres**  | `@data-engineer` | Arquiteto    | Engenharia de dados       |
| **Felix** | `@devops`        | Otimizador   | CI/CD e operações         |
| **Iris**   | `@ux-expert`     | Criador      | Experiência do usuário    |
| **Themis**   | `@aexos-master`   | Orquestrador | Orquestração do framework |

### Ativação de Agentes

```bash
# Ative um agente usando sintaxe @
@dev                # Ativar Vulcan (Desenvolvedor)
@qa                 # Ativar Argus (QA)
@architect          # Ativar Vega (Arquiteto)
@aexos-master        # Ativar Themis (Orquestrador)
...
Nível 2: AEXOS-SQUADS  --> github.com/CyryxLabs (Público/Gratuito)
