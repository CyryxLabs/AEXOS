...
API REST para sincronizar squads com AEXOS e descobrir squads do marketplace.

## Visão Geral

A API de Squads permite:

- **Sync**: Enviar squads locais para a nuvem AEXOS
...
Obtenha sua chave de API em: https://cyryx.ai/settings/api-keys

## Endpoints

### Sincronizar Squad

Enviar uma definição de squad para AEXOS.
...
```bash
curl -X PATCH https://api.cyryx.ai/api/squads/my-squad \
  -H "Authorization: Bearer sk_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{"isPublic": true}'
```
...
```bash
curl -X DELETE https://api.cyryx.ai/api/squads/my-old-squad \
  -H "Authorization: Bearer sk_your_api_key"
```
...
```json
{
  ...
  "data": {
    "valid": true,
    "errors": [],
    "warnings": ["Missing recommended field: description", "Missing cyryx.minVersion field"]
  }
}
```
...
O comando `*sync-squad-cyryx` usa esta API:
...
```bash
export AEXOS_API_TOKEN="sk_your_api_key"
```
...
{
  "info": {
    "name": "AEXOS Squads API",
    "description": "API REST para o Marketplace de Squads AEXOS",
...
Salve o JSON acima como `aexos-squads-api.postman_collection.json` e importe no Postman.
