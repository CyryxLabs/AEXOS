...
> Schema definitions for AEXOS agents and tasks with Auto-Claude capabilities.
...
- **V2 (Legacy)**: Current AEXOS format without `autoClaude` section
...
```bash
node validate-v3-schema.js .aexos-core/development/agents/dev.md
```
...
```bash
node validate-v3-schema.js --diff .aexos-core/development/agents/dev.md
```
...
```bash
node validate-v3-schema.js .aexos-core/development/agents/dev.md --strict
```
...
- Migration Stories: `docs/stories/aexos-core-ade/epic-2-migration-v2-v3.md`
...
_AEXOS V3 Schemas - AEXOS Framework_
