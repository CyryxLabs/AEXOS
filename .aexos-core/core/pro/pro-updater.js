 ...
 * - Checking compatibility with installed aexos-core
 * - Updating the package via the project's package manager
 * - Re-scaffolding Pro assets after update
 *
 * @module .aexos-core/core/pro/pro-updater
 * @story 122.3 — Implementar aexos pro update
 */
...
/**
 * Get the installed aexos-core version.
 * @param {string} projectRoot
 * @returns {string|null}
 */
function getCoreVersion(projectRoot) {
  const resolvedProjectRoot = path.resolve(projectRoot);
  const versionJsonPath = path.join(resolvedProjectRoot, '.aexos-core', 'version.json');
 ...
 * @param {boolean} [options.includeCoreUpdate=false] - Also update aexos-core
...
async function updatePro(projectRoot, options = {}) {
  ...
  if (!installed) {
    result.error = 'AEXOS Pro is not installed. Run: aexos pro setup';
  ...
  if (isUpToDate && !force) {
    ...
    if (!skipScaffold && !dryRun) {
      const scaffolded = await applyScaffoldStep(
        ...
        result,
        onProgress,
        'AEXOS Pro is up to date, but re-scaffolding failed.',
      );
      if (!scaffolded) {
        return result;
      }
  ...
  // 5. Check compatibility with aexos-core
  ...
  if (requiredCore && coreVersion && !satisfiesPeer(coreVersion, requiredCore)) {
    if (!includeCoreUpdate) {
      result.error = `Pro ${latest.version} requires aexos-core ${requiredCore}, but ${coreVersion} is installed. Run: aexos pro update --include-core`;
      result.actions.push({
        action: 'compat',
        status: 'incompatible',
  ...
  if (dryRun) {
    ...
    if (includeCoreUpdate) {
      const corePackageName = detectCorePackageName(resolvedProjectRoot) || 'aexos-core';
      result.actions.push({
        action: 'core_update',
        status: 'dry_run',
  ...
  if (includeCoreUpdate) {
    onProgress('core', 'Updating aexos-core...');
    try {
      const corePackageName = detectCorePackageName(resolvedProjectRoot) || 'aexos-core';
      ...
      result.error = `Failed to update aexos-core: ${err.message}`;
  ...
  if (!skipScaffold) {
    ...
    const scaffolded = await applyScaffoldStep(
      ...
      result,
      onProgress,
      'AEXOS Pro package updated, but re-scaffolding failed.',
    );
    if (!scaffolded) {
      return result;
    }
...
function formatUpdateResult(result) {
  ...
  if (checkAction?.status === 'up_to_date') {
    lines.push(`\n  ✅ AEXOS Pro is up to date (v${result.previousVersion})`);
