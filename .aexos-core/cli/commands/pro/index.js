 ...
 * Pro Command Module
 *
 * CLI commands for AEXOS Pro license management and feature gating.
 *
 * Subcommands:
 *   aexos pro activate --key <KEY>    Activate a license key
 *   aexos pro status                   Show license status
 *   aexos pro deactivate               Deactivate the current license
 *   aexos pro features                 List all pro features
 *   aexos pro validate                 Force online revalidation
 *   aexos pro setup                    Install or verify the AEXOS Pro package
 *
...
function loadLicenseModules() {
  try {
    ...
    console.error('AEXOS Pro license module not available.');
    console.error('Install AEXOS Pro: aexos pro setup');
    console.error('Or via wrapper: npx aexos-pro install');
    process.exit(1);
  }
}

/**
 * Get AEXOS Core version from package.json
 */
...
async function activateAction(options) {
  ...
  if (!key) {
    ...
    console.error('Usage: aexos pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
    process.exit(1);
  }
  ...
  console.log('\nActivating AEXOS Pro license...');
  console.log(`Key: ${maskKey(key)}`);
  console.log('');

  try {
    const machineId = generateMachineId();
    const cyryxCoreVersion = getCyryxCoreVersion();

    const result = await licenseApi.activate(key, machineId, cyryxCoreVersion);
    ...
    if (fs.existsSync(proSourceDir)) {
      ...
      if (scaffoldProContent) {
        ...
        if (scaffoldResult.success) {
          ...
          console.error('Try running "aexos pro activate" again to retry scaffolding.');
        }
        console.log('');
      }
    } else {
      console.log('Note: AEXOS Pro package not found in node_modules.');
...
function statusAction() {
  ...
  if (!cache) {
    ...
    console.log('  Activate: aexos pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
    console.log('  Purchase: https://cyryx.ai/pro');
    console.log('');
    return;
  }
  ...
  if (info && info.inGrace) {
    ...
    console.log('  Please revalidate your license: aexos pro validate');
  }
...
async function deactivateAction(options) {
  ...
  if (!options.force) {
    console.log('\nDeactivating AEXOS Pro License');
    console.log(`Key: ${maskKey(cache.key)}`);
    console.log('\nThis will:');
  ...
  try {
    ...
    console.log('');
    console.log('To reactivate: aexos pro activate --key <KEY>');
    console.log('');
  } catch (error) {
...
// ---------------------------------------------------------------------------
// aexos pro setup (AC-12: Install-gate)
// ---------------------------------------------------------------------------
...
/**
 * Setup and verify AEXOS Pro installation.
 *
...
async function setupAction(options) {
  ...
  if (options.verify) {
    console.log('Verifying AEXOS Pro installation...\n');
    ...
    try {
      ...
      if (found) {
        console.log('✅ AEXOS Pro content is installed');
      }
      if (!found) {
        console.log('❌ AEXOS Pro is not installed');
        console.log('');
        console.log('Install with:');
        console.log('  aexos pro setup');
        console.log('  # or npx aexos-pro install');
      }
    } catch {
      console.log('❌ AEXOS Pro is not installed');
      console.log('');
...
async function updateAction(options) {
  ...
  try {
    ...
    console.error('Please ensure aexos-core is installed correctly.');
    process.exit(1);
  }
  ...
  if (options.check || options.dryRun) {
    if (!installed) {
      console.log('\n  ❌ AEXOS Pro is not installed. Run: aexos pro setup\n');
      process.exit(1);
    }

    if (options.dryRun) {
      console.log('\n  📋 AEXOS Pro Update Plan');
      console.log('  ───────────────────────');
    ...
    if (installed.version === targetVersion) {
      console.log(`\n  ✅ AEXOS Pro is up to date (v${installed.version})\n`);
      return;
    }

    console.log('\n  🔄 AEXOS Pro update available');
    console.log(`  Installed:    v${installed.version}`);
    console.log(`  Target:       v${targetVersion}`);
    console.log('  Run:          aexos pro update');
    console.log('');
    return;
  }
...
/**
 * Create the `aexos pro` command with all subcommands.
 * @returns {Command}
 */
function createProCommand() {
  const proCmd = new Command('pro').description('AEXOS Pro license management');
  ...
  proCmd
    .command('setup')
    .description('Authenticate and install AEXOS Pro')
  ...
  proCmd
    .command('update')
    .description('Update AEXOS Pro to latest version and sync assets')
    .option('--check', 'Check for updates without applying')
    .option('--dry-run', 'Show update plan without executing')
    .option('-f, --force', 'Force reinstall even if up-to-date')
    .option('--include-core', 'Also update aexos-core')
    .option('--skip-scaffold', 'Skip re-scaffolding assets after update')
    .action(updateAction);

  // aexos pro buyer — Cohort admin operations (Story 123.8)
  proCmd.addCommand(createBuyerCommand());

  // aexos pro seats — Self-service seat management (EPIC-PRO-17)
