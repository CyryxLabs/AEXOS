 ...
 *
 *   aexos pro seats list [--email E] [--token T]
 *   aexos pro seats release <activationId> [--email E] [--token T]
 *
...
async function resolveAccessToken(options, licenseApi) {
  ...
  if (!email || !password) {
    ...
    console.error('Use --email with AEXOS_PRO_PASSWORD, --token, or AEXOS_PRO_EMAIL/AEXOS_PRO_PASSWORD.');
    process.exit(1);
    return null;
  }
...
async function listSeatsAction(options) {
  ...
  try {
    ...
    for (const seat of result.seats) {
      ...
      if (seat.cyryxVersion) {
        console.log(`    Version:   ${seat.cyryxVersion}`);
      }
...
async function releaseSeatAction(activationId, options) {
  ...
  if (!activationId) {
    console.error('Usage: aexos pro seats release <activationId>');
    process.exit(1);
    return;
  }
