 ...
 *
 * CLI commands for AEXOS Pro buyer validation and management.
 * Consumes aexos-license-server via pro/license/license-api.js.
 *
 * Subcommands (Wave 1 — this file):
 *   aexos pro buyer validate --email <E> [--json]
 *   aexos pro buyer validate-batch --file <F> [--concurrency N] [--json]
 *
 * Subcommands (Wave 2 — depends on POST /api/v1/admin/buyers/register in
 * aexos-license-server; not yet implemented):
 *   aexos pro buyer register --email <E> --name <N> [--cpf <C>] [--yes] [--json]
 *
...
function loadClient() {
  try {
    ...
    console.error('Erro: módulo AEXOS Pro license não disponível.');
...
function classifyError(err) {
  ...
  if (code === 'NETWORK_ERROR' || code === 'SERVER_ERROR') {
    return {
      ...
      hint: 'Para mais detalhes: AEXOS_DEBUG=true aexos pro buyer validate ...',
    };
  }
...
// ---------------------------------------------------------------------------
// aexos pro buyer validate-batch
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// aexos pro buyer register — Wave 2 stub (hidden until endpoint exists)
// ---------------------------------------------------------------------------
  ...
  if (options.json) {
    process.stdout.write(
      JSON.stringify({
        ...
        reason:
          'Endpoint POST /api/v1/admin/buyers/register em aexos-license-server ainda não foi implementado.',
    ...
    process.stderr.write(
      '\nOperação `register` pendente (Wave 2 da Story 123.8).\n' +
        'Depende do endpoint POST /api/v1/admin/buyers/register no repo aexos-license-server,\n' +
...
/**
 * Create the `aexos pro buyer` subcommand group.
 * @returns {Command}
 */
function createBuyerCommand() {
  const cmd = new Command('buyer').description(
    'Validar e gerenciar buyers AEXOS Pro (Cohort admin)'
  );

  cmd
    .command('validate')
    .description('Verificar se um email é comprador AEXOS Pro')
