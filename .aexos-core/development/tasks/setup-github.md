...
Configure complete GitHub DevOps infrastructure for user projects created with AEXOS. This task copies GitHub Actions workflows, configures CodeRabbit, sets up branch protection, and manages secrets.
...
# Check idempotency
if (Test-Path ".aexos/devops-setup-report.yaml") {
...
$ciTemplate = Get-Content ".aexos-core/infrastructure/templates/github-workflows/ci.yml.template" -Raw
...
Copy-Item ".aexos-core/infrastructure/templates/github-workflows/pr-automation.yml.template" `
  -Destination ".github/workflows/pr-automation.yml"
Write-Host "✅ Installed pr-automation.yml"

# Copy release.yml
Copy-Item ".aexos-core/infrastructure/templates/github-workflows/release.yml.template" `
...
$coderabbitConfig = Get-Content ".aexos-core/infrastructure/templates/coderabbit.yaml.template" -Raw
...
New-Item -ItemType Directory -Path ".aexos" -Force | Out-Null

$report | Out-File -FilePath ".aexos/devops-setup-report.yaml" -Encoding utf8

Write-Host "✅ Setup report saved to .aexos/devops-setup-report.yaml"
```
...
║     git add .github/ .coderabbit.yaml .aexos/                              ║
║     git commit -m "chore: add DevOps configuration [Story 5.10]"          ║
║     git push                                                               ║
║                                                                            ║
║  Report saved: .aexos/devops-setup-report.yaml                             ║
║                                                                            ║
╚═══════════════════════════════════════════════════════════════════════════╝

— Polaris, DevOps configured with confidence 🚀
```
