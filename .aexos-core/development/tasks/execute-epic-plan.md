...
4. **Error:** State file corrupted
   ...
   - **Recovery:** Restore from .aexos/epic-{epicId}-state.yaml.bak
...
- Template `epic-orchestration.yaml` exists in `.aexos-core/development/workflows/`
- Inner loop `development-cycle.yaml` exists in `.aexos-core/development/workflows/`
...
**1. Load state** from `.aexos/epic-{epicId}-state.yaml`
...
State preserved at: .aexos/epic-{epicId}-state.yaml
...
PROCEDURE execute_wave(wave, stories, state):
  ...
  Task tool call:
    ...
    subagent_type: "aexos-architect"  # or whatever the gate agent is
    prompt: |
...
- `@pm` (Janus/Bob) - Primary orchestrator

This task spawns:
- `@po` (Themis) - Story validation, checkpoints
- `@dev` (Vulcan) - Story implementation (via development-cycle)
- `@architect` (Vega) - Wave gates, final gate
- `@devops` (Polaris) - Branch merge, push
- `@qa` (Argus) - Quality gates (via development-cycle)
