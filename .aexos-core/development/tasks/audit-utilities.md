...
- **Tool:** code-analyzer
  ...
  - **Source:** .aexos-core/utils/code-analyzer.js
...
- **Script:** analyze-codebase.js
  ...
  - **Language:** JavaScript
  - **Location:** .aexos-core/scripts/analyze-codebase.js
...
Systematically audit all utilities in `.aexos-core/scripts/` to determine their functional status, classify them as WORKING/FIXABLE/DEPRECATED, and generate actionable recommendations for maintenance and cleanup.
...
for util in .aexos-core/scripts/*.js; do
  name=$(basename $util .js)
  count=$(grep -r "$name" .aexos-core/agents .aexos-core/tasks Squads 2>/dev/null | wc -l)
