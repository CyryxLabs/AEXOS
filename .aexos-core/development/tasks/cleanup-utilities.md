...
mkdir -p .backups
cp -r .aexos-core/utils ".backups/utils.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.*utility-name" .aexos-core/agents .aexos-core/tasks .aexos-core/workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name" .aexos-core/agents/ .aexos-core/tasks/ .aexos-core/core-config.yaml
...
1. **Copy file back**:
   ```bash
   cp .aexos-core/utils-archive/utility-name.js .aexos-core/scripts/
   ```
...
4. **Test thoroughly**:
   ```bash
   node .aexos-core/scripts/test-utilities.js
   ```
...
**Immediate Rollback**:
```bash
rm -rf .aexos-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .aexos-core/utils
```

**Selective Restoration**:
```bash
cp .aexos-core/utils-archive/specific-utility.js .aexos-core/scripts/
```
...
git mv .aexos-core/scripts/utility-name.js .aexos-core/utils-archive/

# Or for multiple files:
git mv .aexos-core/scripts/{aexos-validator-fixed.js,aexos-validator-refactored.js} .aexos-core/utils-archive/
```
...
git mv .aexos-core/scripts/aexos-validator.test.js tests/utils/
```
...
1. aexos-validator-fixed.js - Duplicate of aexos-validator.js
2. aexos-validator-refactored.js - Duplicate of aexos-validator.js
...
...
```bash
node .aexos-core/scripts/aexos-validator.js
```
...
for util in $(ls .aexos-core/utils-archive/*.js); do
  name=$(basename $util .js)
  refs=$(grep -r "require.*$name" .aexos-core/agents .aexos-core/tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
...
2. `.aexos-core/utils-archive/ARCHIVE-README.md` - Archive documentation
