...
steps:
  ...
  2_check_dependencies:
    ...
    checks:
      ...
      - Does this change affect IDE sync? (.aexos-core/infrastructure/scripts/ide-sync/)
      ...
      - Does this change affect other agents? (.aexos-core/development/agents/)
...
steps:
  ...
  3_handle_edge_cases:
    ...
    known_pitfalls:
      - Email addresses inside strings may match rename patterns (Issue #159: security@aexos-squads/core.dev)
  ...
  4_regenerate_manifests:
    ...
    action: |
      ...
      This regenerates .aexos-core/install-manifest.yaml
    when: Any file in .aexos-core/ or packages/ is created, modified, or deleted

  5_run_ide_sync:
    condition: Changes affect agent definitions or IDE sync system
    action: |
      Run: node .aexos-core/infrastructure/scripts/ide-sync/index.js sync --verbose
      Verify all IDEs sync without errors
    when: Changes to .aexos-core/development/agents/ or ide-sync/
```
...
- **Pitfall:** `@aexos-squads/core` inside email `security@aexos-squads/core.dev` was corrupted
