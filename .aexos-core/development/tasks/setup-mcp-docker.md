...
Configure Docker MCP Toolkit as the primary MCP infrastructure for AEXOS, using **HTTP transport** instead of stdio to avoid timeout issues during gateway initialization.
...
Configure Docker MCP Toolkit as the primary MCP infrastructure for AEXOS, replacing 1MCP with the containerized gateway approach. This enables:
