...
**Purpose**: Standardize the orchestrator/executor split for AEXOS workflows. The active AEXOS runtime keeps authority over story interpretation, acceptance criteria validation, constitutional gates, review, and story updates while a separate CLI runtime performs only the implementation attempt.
...
```bash
aexos-delegate codex -t <slug> -f <prompt_file> -d <workdir>
```
...
```text
STATUS=started
RUN_DIR=.aexos/external-runs/<timestamp>-<slug>
PID=<pid>
