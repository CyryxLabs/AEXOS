...
Read the current permission mode from `.aexos/config.yaml` under `permissions.mode`.
...
Update `.aexos/config.yaml` with the new `permissions.mode` value.
The `PermissionMode._saveToConfig()` method handles this, including creating the `.aexos/` directory if needed.
...
```javascript
const { cycleMode } = require('./.aexos-core/core/permissions');
...
- If `.aexos/config.yaml` cannot be read, start from default `ask` mode
...
version: 1.0.0
dependencies:
  - .aexos-core/core/permissions/permission-mode.js
