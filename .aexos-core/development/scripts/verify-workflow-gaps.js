 ...
 * Usage: node .aexos-core/development/scripts/verify-workflow-gaps.js
 *
 * @version 1.0.0
 */
...
function verifyGap1() {
  ...
  const steps = require('../../../.aexos-core/core/elicitation/workflow-elicitation.js');
  ...
  const template = fs.readFileSync(path.join(ROOT, '.aexos-core/product/templates/workflow-template.yaml'), 'utf-8');
  assert(
...
async function verifyGap2() {
  ...
  try {
    ({ WorkflowValidator } = require('../../../.aexos-core/development/scripts/workflow-validator'));
  ...
  const goodResult = await validator.validate(path.join(ROOT, '.aexos-core/development/workflows/greenfield-service.yaml'));
  assert(
  ...
  const strictResult = await strictValidator.validate(path.join(ROOT, '.aexos-core/development/workflows/greenfield-service.yaml'));
  assert(
  ...
  const { SquadValidator } = require('../../../.aexos-core/development/scripts/squad/squad-validator');
  ...
  const FrameworkAnalyzer = require('../../../.aexos-core/infrastructure/scripts/framework-analyzer');
  ...
  assert(
    fs.existsSync(path.join(ROOT, '.aexos-core/development/tasks/validate-workflow.md')),
    '2.12 validate-workflow.md task file exists',
    'File not found',
  );

  // 2.13 aexos-master has validate-workflow command
  const masterMd = fs.readFileSync(path.join(ROOT, '.aexos-core/development/agents/aexos-master.md'), 'utf-8');
  assert(
    masterMd.includes('name: validate-workflow'),
    '2.13a aexos-master has validate-workflow command',
    'Command not found in aexos-master.md',
  );
  assert(
    masterMd.includes('validate-workflow.md'),
    '2.13b aexos-master has validate-workflow.md in dependencies',
...
async function verifyGap3() {
  ...
  try {
    ({ WorkflowStateManager } = require('../../../.aexos-core/development/scripts/workflow-state-manager'));
  ...
  try {
    ...
    const wfContent = fs.readFileSync(path.join(ROOT, '.aexos-core/development/workflows/greenfield-service.yaml'), 'utf-8');
    ...
    const patterns = yaml.load(fs.readFileSync(path.join(ROOT, '.aexos-core/data/workflow-patterns.yaml'), 'utf-8'));
    assert(
    ...
    const schemaContent = fs.readFileSync(path.join(ROOT, '.aexos-core/data/workflow-state-schema.yaml'), 'utf-8');
    ...
    assert(
      fs.existsSync(path.join(ROOT, '.aexos-core/development/tasks/run-workflow.md')),
      '3.21 run-workflow.md task file exists',
      'File not found',
    );

    // 3.22 aexos-master has run-workflow command
    const masterMd = fs.readFileSync(path.join(ROOT, '.aexos-core/development/agents/aexos-master.md'), 'utf-8');
    assert(
      masterMd.includes('name: run-workflow'),
      '3.22a aexos-master has run-workflow command',
      'Command not found',
    );
    assert(
      masterMd.includes('run-workflow.md'),
      '3.22b aexos-master has run-workflow.md in dependencies',
      'Dependency not found',
    );
...
async function verifyGap4() {
  ...
  const { WorkflowValidator, WorkflowValidationErrorCodes } = require('../../../.aexos-core/development/scripts/workflow-validator');
  ...
  try {
    ...
    for (const taskFile of taskFiles) {
      const content = fs.readFileSync(path.join(ROOT, '.aexos-core/development/tasks', taskFile), 'utf-8');
      assert(
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('  AEXOS Workflow Gaps — Verification Suite');
