...
const SCAN_CONFIG = [
  { category: 'tasks', basePath: '.aexos-core/development/tasks', glob: '**/*.md', type: 'task' },
  { category: 'templates', basePath: '.aexos-core/product/templates', glob: '**/*.{yaml,yml,md}', type: 'template' },
  { category: 'scripts', basePath: '.aexos-core/development/scripts', glob: '**/*.{js,mjs}', type: 'script' },
  { category: 'modules', basePath: '.aexos-core/core', glob: '**/*.{js,mjs}', type: 'module' },
  { category: 'agents', basePath: '.aexos-core/development/agents', glob: '**/*.{md,yaml,yml}', type: 'agent' },
  { category: 'checklists', basePath: '.aexos-core/development/checklists', glob: '**/*.md', type: 'checklist' },
  { category: 'data', basePath: '.aexos-core/data', glob: '**/*.{yaml,yml,md}', type: 'data' },
  { category: 'workflows', basePath: '.aexos-core/development/workflows', glob: '**/*.{yaml,yml}', type: 'workflow' },
  { category: 'utils', basePath: '.aexos-core/core/utils', glob: '**/*.js', type: 'util' },
  { category: 'tools', basePath: '.aexos-core/development/tools', glob: '**/*.{md,js,sh}', type: 'tool' },
  { category: 'infra-scripts', basePath: '.aexos-core/infrastructure/scripts', glob: '**/*.js', type: 'script' },
  { category: 'infra-tools', basePath: '.aexos-core/infrastructure/tools', glob: '**/*.{yaml,yml,md}', type: 'tool' },
  { category: 'product-checklists', basePath: '.aexos-core/product/checklists', glob: '**/*.md', type: 'checklist' },
  { category: 'product-data', basePath: '.aexos-core/product/data', glob: '**/*.{yaml,yml,md}', type: 'data' },
  // Framework documentation under `.aexos-core/core/docs/` is reference material
  ...
  { category: 'core-docs', basePath: '.aexos-core/core/docs', glob: '**/*.md', type: 'docs' },
];
...
/**
 * Normalize a timestamp read back from the registry to an ISO-8601 string.
 *
 * Timestamps are written quoted, so `yaml.load` normally returns them as
 * strings. A hand-edited (unquoted) value would be resolved by js-yaml's
 * `!!timestamp` type into a Date instead; coercing here keeps the value a
 * string so re-dumping it produces the same bytes it came from.
 *
 * @param {string|Date} value - Timestamp as loaded from the registry
...
const KNOWN_AGENTS = [
  ...
  'analyst', 'data-engineer', 'ux-design-expert', 'aexos-master',
];
...
function populate(options = {}) {
  const verbose = options.verbose || process.argv.includes('--verbose') || process.env.AEXOS_DEBUG === 'true';
  ...
  //
  // Also preserve `lastVerified` for entities whose content is unchanged: a
  // regen that finds the same checksum has re-verified the same bytes, so
  // re-stamping the clock would record a change that never happened and make
  // the registry file differ on every run (invalidating the install manifest,
  // which hashes raw bytes). Entities that are new or whose checksum moved
  // still get a fresh timestamp below.
  let existingYaml;
  let existingRegistry;
  try {
    existingYaml = fs.readFileSync(REGISTRY_PATH, 'utf8');
    existingRegistry = yaml.load(existingYaml);
    if (existingRegistry && existingRegistry.entities) {
      let preservedTimestamps = 0;
      for (const [category, entities] of Object.entries(existingRegistry.entities)) {
        ...
        for (const [entityId, entity] of Object.entries(entities)) {
          ...
          const current = allEntities[category][entityId];
          if (current && entity.lastVerified && entity.checksum === current.checksum) {
            current.lastVerified = toIsoString(entity.lastVerified);
            preservedTimestamps++;
          }
        }
      }
      console.log('[IDS] Preserved invocationExamples from existing registry');
      console.log(`[IDS] Preserved lastVerified for ${preservedTimestamps} unchanged entities`);
    }
  } catch {
  ...
  // Sync the self-entry for `.aexos-core/data/entity-registry.yaml` so the
  ...
  const buildRegistry = (lastUpdated) => {
    const built = {
      metadata: {
        version: '1.0.0',
        lastUpdated,
    ...
    syncSelfRegistryEntry(built, lastUpdated, SELF_CHECKSUM_SENTINEL);
    return built;
  };

  const dumpRegistry = (value) => yaml.dump(value, {
    lineWidth: 120,
    noRefs: true,
    sortKeys: false,
  });

  // `metadata.lastUpdated` records when the registry last changed, not when it
  // was last regenerated. Build first with the previous timestamp carried
  // forward: if the resulting bytes match the file already on disk, nothing
  // moved and the registry stays byte-identical across regens (the install
  // manifest hashes raw bytes, so a purely cosmetic timestamp bump would
  // invalidate it after every run). Any real difference re-stamps to now.
  const previousLastUpdated = existingRegistry?.metadata?.lastUpdated;
  const candidateLastUpdated = previousLastUpdated
    ? toIsoString(previousLastUpdated)
    : new Date().toISOString();

  let registry = buildRegistry(candidateLastUpdated);
  let yamlContent = dumpRegistry(registry);

  if (existingYaml !== undefined && yamlContent !== existingYaml) {
    registry = buildRegistry(new Date().toISOString());
    yamlContent = dumpRegistry(registry);
  } else if (existingYaml !== undefined) {
...
/**
 * Sync the self-entry for `.aexos-core/data/entity-registry.yaml`.
 *
...
function syncSelfRegistryEntry(registry, lastUpdated, sentinelChecksum) {
  ...
  // just wrote — the `.js` module at .aexos-core/core/doctor/checks/entity-registry.js
  // gets its own legitimate entry under `entities.modules` (or wherever
  // the modules scan places it) and is not self-referential.
  const expectedPath = '.aexos-core/data/entity-registry.yaml';
