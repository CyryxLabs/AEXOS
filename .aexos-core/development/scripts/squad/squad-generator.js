...
/**
 * Default AEXOS minimum version
 * @constant {string}
 */
const DEFAULT_AEXOS_MIN_VERSION = '2.1.0';
...
cyryx:
  minVersion: "${config.cyryxMinVersion || DEFAULT_AEXOS_MIN_VERSION}"
  type: squad
...
${config.description || 'Custom AEXOS squad.'}
...
This squad ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'does not inherit'} the core AEXOS configuration.
...
> This file ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'is independent of'} the core AEXOS coding standards.
...
- Node.js >= 18.x
- AEXOS >= ${config.cyryxMinVersion || DEFAULT_AEXOS_MIN_VERSION}
...
class SquadGenerator {
   ...
   * @param {string} [config.cyryxMinVersion] - Minimum AEXOS version
  ...
  async generate(config) {
    ...
    const fullConfig = {
      ...
      cyryxMinVersion: config.cyryxMinVersion || DEFAULT_AEXOS_MIN_VERSION,
