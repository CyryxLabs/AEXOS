...
class IntegrationSuggester {
  ...
  suggest(analysis, provider) {
    ...
    suggestions.push({
      type: 'add_step',
      priority: 'high',
      title: 'Add AEXOS Build Orchestration',
      description: 'Integrate AEXOS build orchestrator for intelligent parallel builds',
    ...
    if (!analysis.hasSecurityScan) {
      suggestions.push({
        ...
        description: 'Add AEXOS PR Review AI for security analysis',
  ...
  getCYRYXBuildStep(provider) {
    const steps = {
      'github-actions': `- name: AEXOS Build Orchestration
  run: npx aexos build --parallel --smart-cache
  env:
    AEXOS_CI: true`,
  ...
  stage: build
  script:
    - npx aexos build --parallel --smart-cache
  variables:
    AEXOS_CI: "true"`,

      jenkins: `stage('AEXOS Build') {
  steps {
    sh 'npx aexos build --parallel --smart-cache'
  }
  environment {
    AEXOS_CI = 'true'
  }
      ...
      circleci: `- run:
    name: AEXOS Build Orchestration
    command: npx aexos build --parallel --smart-cache
    environment:
      AEXOS_CI: true`,
    };
  ...
  run: npx aexos test --discover --coverage`,

      'gitlab-ci': `test:
  stage: test
  script:
    - npx aexos test --discover --coverage
  coverage: '/Coverage: \\d+\\.\\d+%/'`,

      jenkins: `stage('Test') {
  steps {
    sh 'npx aexos test --discover --coverage'
  }
}`,

      circleci: `- run:
    name: Run Tests
    command: npx aexos test --discover --coverage`,
    };
  ...
  run: npx aexos review --security-only
  continue-on-error: true`,

      'gitlab-ci': `security_scan:
  stage: test
  script:
    - npx aexos review --security-only
  allow_failure: true`,

      jenkins: `stage('Security Scan') {
  steps {
    sh 'npx aexos review --security-only'
  }
  post {
    failure {
      ...
      circleci: `- run:
    name: Security Scan
    command: npx aexos review --security-only
    when: always`,
    };
