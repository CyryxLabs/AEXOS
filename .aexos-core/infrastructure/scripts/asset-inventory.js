...
/**
 * AEXOS Asset Inventory Generator
 * Story 2.1: Creates comprehensive inventory of all AEXOS assets
 *
 * Usage:
...
const ASSET_PATHS = {
  ...
  templates: '.aexos-core/product/templates',
  checklists: '.aexos-core/product/checklists',
  scripts: '.aexos-core/infrastructure/scripts',
  schemas: '.aexos-core/schemas',
  data: '.aexos-core/development/data',
};
...
function formatConsoleOutput(report, verbose = false) {
  ...
  lines.push('');
  lines.push('═══════════════════════════════════════════════════════════');
  lines.push('  AEXOS Asset Inventory');
...
Options:
  ...
  // Find project root (look for .aexos-core directory)
  let rootPath = process.cwd();
  while (rootPath !== '/') {
    try {
      await fs.access(path.join(rootPath, '.aexos-core'));
      break;
    } catch {
  ...
  if (rootPath === '/') {
    console.error('Error: Could not find .aexos-core directory. Run from project root.');
    process.exit(1);
  }
