...
const CORE_CONFIG_FILE = '.aexos-core/core-config.yaml';
 ...
 * - .aexos-core/core/** gets expanded one level deep
...
function expandProtectedPaths(protectedPaths, exceptions, projectRoot) {
  ...
  for (const globPath of protectedPaths) {
    if (globPath === '.aexos-core/core/**') {
      const { dirs, files } = expandOneLevel('.aexos-core/core', projectRoot);
