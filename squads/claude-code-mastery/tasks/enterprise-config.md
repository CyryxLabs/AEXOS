...
Saida: |
  ...
  - compliance_rules: Framework-specific deny rules and policy flags merged into the base managed settings
  - deployment_instructions: Platform-by-platform installation steps and deployment paths
  - verification_commands: Commands proving the managed files are present and loaded on each platform
  - rollback_procedure: The documented steps for removing the managed configuration
Checklist:
  - "[ ] Phase 1: managed-settings.json created with deny-first rules, env block, announcements and the managed-only policy keys"
  - "[ ] Phase 1: disableBypassPermissionsMode set to disable"
  - "[ ] Phase 2: MDM or OS-level policy artifacts produced for each target platform"
  - "[ ] Phase 3: managed-mcp.json created with the approved server list and the chosen allowlisting strategy"
  - "[ ] Phase 4: rules for every specified compliance framework generated and merged into the base configuration"
  - "[ ] Phase 5: deployment artifacts generated per platform, with installation, verification and rollback documentation"
  - "[ ] No real API key, token or credential written into any managed file"
  - "[ ] allowManagedPermissionRulesOnly not set to true without comprehensive deny rules alongside it"
  - "[ ] Rollback procedure documented before any managed-settings.json deployment"
  - "[ ] No compliance framework specified and then left without its rules"
---
