...
Saida: |
  ...
  - pattern_rules: .claude/rules/ files — code-style, architecture, anti-patterns, legacy-areas — with path activation
  - deny_rules: settings.json deny entries protecting the critical paths, plus allow exceptions
  - workflow_integration: Hook alignment with existing linters, git conventions and notification hooks
  - brownfield_setup_result: YAML summary with project analysis, files created, paths protected and disruption risk
Checklist:
  - "[ ] Phase 1: source tree scanned and coding, architectural and naming patterns inventoried without changing any file"
  - "[ ] Phase 2: existing CI/CD, code-quality, testing tooling and package.json scripts detected and documented"
  - "[ ] Phase 3: CLAUDE.md written from the detected patterns, incorporating the team conventions doc when supplied, kept under 200 lines"
  - "[ ] Phase 4: .claude/rules/ pattern files created with path-based activation for legacy and domain-specific areas"
  - "[ ] Phase 5: critical paths collected from the user and auto-detection, and deny rules configured in settings.json"
  - "[ ] Phase 5: deny rules verified not to block legitimate development work"
  - "[ ] Phase 6: hooks aligned with existing linters, git commit/branch conventions documented, notification hooks set for critical operations"
  - "[ ] Existing .claude/settings.json deny rules merged rather than replaced"
  - "[ ] A manually maintained CLAUDE.md was not overwritten without asking"
  - "[ ] No existing workflow disrupted — disruption risk stated in the result"
---
