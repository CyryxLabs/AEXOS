...
Saida: |
  ...
  - allow_rules: Tool(specifier) entries covering the detected development workflows
  - deny_rules: The mandatory baseline plus project-type-specific denies (AEXOS, infrastructure, database, docker)
  - ask_rules: Prompts for git push, destructive git operations and edits to critical config files
  - mcp_and_agent_permissions: Allow, ask or deny per MCP server and per subagent
  - strategy_document: The complete permissions JSON, rule counts, evaluation examples and the security coverage checklist
Checklist:
  - "[ ] Phase 1: sensitivity profile assessed against the weighted factor table"
  - "[ ] Phase 1: sensitive file patterns scanned for and safe operation patterns identified"
  - "[ ] Phase 2: permission mode selected via the decision tree, presented with rationale and open to override"
  - "[ ] Phase 3: allow rules built with Tool(specifier) syntax and customized to the detected project needs"
  - "[ ] Phase 4: mandatory deny rules included in full, plus the project-type-specific denies"
  - "[ ] Phase 4: ask rules set for git push, hard resets and edits to package.json or tsconfig.json"
  - "[ ] Phase 5: every configured MCP server given an allow, ask or deny rule, and subagent permissions set"
  - "[ ] Deny rules for .env and secrets present — the non-negotiable baseline"
  - "[ ] No destructive bash command in the allow list, no pipe-to-shell pattern allowed, and bypassPermissions not recommended for team or enterprise"
  - "[ ] Evaluation examples included showing how common operations resolve under deny > ask > allow"
---
...
|-------------|----------------------|
| AEXOS | `Edit(.aexos-core/core/**)`, `Edit(.aexos-core/constitution.md)`, `Edit(bin/aexos.js)` |
