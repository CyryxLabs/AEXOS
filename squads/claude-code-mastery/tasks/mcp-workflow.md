...
Saida: |
  ...
  - transport_config: stdio or HTTP Streamable per server, with auth and env-var handling
  - validation_results: Tool availability per server, sample call results and the diagnosis for any failures
  - documentation: CLAUDE.md MCP section and .claude/rules/mcp-usage.md with the native > MCP > Bash priority
  - mcp_workflow_result: YAML summary with servers, transports, tool counts, token costs, budget status and files modified
Checklist:
  - "[ ] Phase 1: project scanned for stack indicators, cross-referenced against the MCP catalog, and gaps against current servers listed"
  - "[ ] Phase 2: token cost calculated per proposed server, summed, compared to the budget limit, and low-value servers ranked for removal if over"
  - "[ ] Phase 3: scope decided per server (project vs global), entries written with correct structure and the JSON validated after writing"
  - "[ ] Phase 4: local vs remote determined per server, transport configured accordingly, and API keys set via environment variables rather than hardcoded"
  - "[ ] Phase 5: each server verified for tool availability with a minimal test call, and failures diagnosed against binary, key, port and network"
  - "[ ] Phase 6: CLAUDE.md MCP section added or updated with servers, purposes and tool-selection priority"
  - "[ ] Phase 6: .claude/rules/mcp-usage.md created if absent, and server-specific gotchas documented"
  - "[ ] Servers requiring an unavailable API key or an uninstallable binary skipped and documented rather than half-configured"
  - "[ ] Total MCP budget kept at or below 15,000 tokens"
  - "[ ] Context budget documented alongside the final configuration"
---
