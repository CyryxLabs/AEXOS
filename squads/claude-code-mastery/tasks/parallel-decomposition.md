...
Saida: |
  ...
  - agent_configs: Per subtask the agent definition used or created, model tier, tool permissions and max_turns
  - merge_strategy: File or directory ownership, the merge process, conflict resolution and the branch merge sequence
  - monitoring_plan: Progress checkpoints, timeout thresholds, failure handling and the rollback plan
  - parallel_decomposition_result: YAML summary with wave count, sequential vs parallel time, speedup and critical path
Checklist:
  - "[ ] Phase 1: task broken into atomic subtasks that are single-agent, independently testable and artifact-producing, each classified for complexity, model and time"
  - "[ ] Phase 2: dependency type determined for every subtask pair and a dependency graph built"
  - "[ ] Phase 2: circular dependencies checked for, and the critical path calculated"
  - "[ ] Phase 3: subtasks grouped into waves from the dependency graph, execution mode chosen per wave and the timeline documented"
  - "[ ] Phase 4: agent chosen or created per subtask, model set by complexity, tools restricted to the minimum, max_turns set"
  - "[ ] Phase 5: file or directory ownership assigned per agent and the merge, conflict-resolution and integration-test process defined"
  - "[ ] Phase 6: progress checkpoints, per-subtask timeouts, retry and escalation rules, and the rollback plan defined"
  - "[ ] Halted if a circular dependency was found or if all subtasks proved sequentially dependent"
  - "[ ] Subtask count kept at 10 or fewer, and no design left where multiple agents write the same file"
  - "[ ] Estimated cost kept within the stated budget"
---
