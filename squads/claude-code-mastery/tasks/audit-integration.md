...
Saida: |
  ...
  - hook_health: Each hook classified HEALTHY, DEGRADED, BROKEN or MISSING from static analysis
  - mcp_status: Each server categorized CONNECTED, CONFIGURED, MISCONFIGURED, MISSING, or N/A
  - health_score: Weighted integration score 0-100 with the grade letter from the threshold table
  - findings: Critical, Warning and Info findings, each paired with a recommendation
  - quick_fixes: Numbered commands or actions that resolve the top issues
Checklist:
  - "[ ] Phase 1: .claude/ completeness checked against the weighted component table, recording present/missing, size and last modified"
  - "[ ] Phase 2: settings.json parsed and checked for JSON validity, allow/deny contradictions, path accuracy, missing dangerous-command denies and wildcard allows"
  - "[ ] Phase 3: every rule file checked for frontmatter, non-empty actionable content and glob patterns that match real files"
  - "[ ] Phase 4: each hook checked for registration, file existence, parse errors, executable bit and timeout risk"
  - "[ ] Phase 5: MCP servers checked for complete configuration, command on PATH and empty credential env vars"
  - "[ ] Phase 6: weighted score computed and a grade assigned from the threshold table"
  - "[ ] No project file modified — the audit stayed read-only"
  - "[ ] No hook script executed and no MCP connection attempted"
  - "[ ] No credential or secret value reproduced in the report"
  - "[ ] Critical findings listed with specific recommendations and quick fixes"
---
...
6. **AEXOS compatibility**: if AEXOS project, verify L1/L2 protection rules present
