...
Saida: |
  ...
  - claude_md_updated: The trimmed CLAUDE.md with reference comments where sections were moved
  - memory_review: Auto-memory files listed with sizes, staleness and duplication against CLAUDE.md
  - compaction_recommendation: Recommended CLAUDE_AUTOCOMPACT_PCT_OVERRIDE, PreCompact hook status and max output tokens guidance
  - budget_report: Before/after lines, sections and tokens, plus the always-loaded context total and the savings summary
Checklist:
  - "[ ] Phase 1: all CLAUDE.md files located and measured for lines, sections, estimated tokens and @imports"
  - "[ ] Phase 1: every section categorized core, conditional, reference or redundant with a recommendation"
  - "[ ] Phase 2: conditional sections moved into .claude/rules/ files with matching path globs, formatting and code examples preserved"
  - "[ ] Phase 2: moved sections removed from CLAUDE.md and replaced with a reference comment"
  - "[ ] Phase 3: frontmatter verified on every new rule file and glob patterns tested against real project files"
  - "[ ] Phase 3: rules organized into subdirectories when numerous, and always-on rules that should be conditional converted"
  - "[ ] Phase 4: auto-memory files listed, checked for staleness and duplication, with cleanup suggested rather than applied"
  - "[ ] Phase 5: compaction threshold recommended from the project size table, PreCompact hook and max output tokens checked"
  - "[ ] No CLAUDE.md content deleted without being moved to rules or confirmed redundant, and core instructions kept intact"
  - "[ ] Before/after comparison generated showing the token savings"
---
