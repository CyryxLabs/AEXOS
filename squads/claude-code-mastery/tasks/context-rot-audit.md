...
Saida: |
  ...
  - rules_audit: Active, orphaned and missing-coverage rules with their path patterns
  - memory_audit: Stale agent-memory entries, contradictions and MEMORY.md size against the 200-line limit
  - rot_score: The computed score with its severity band (green, yellow, orange, red)
  - remediation_plan: P0-P4 prioritized fix list with effort and impact, plus what was auto-fixed
Checklist:
  - "[ ] Phase 1: CLAUDE.md total and per-section line counts measured, largest sections and duplicates identified against the size thresholds"
  - "[ ] Phase 2: every file path and command referenced in CLAUDE.md and rules checked for existence, missing ones reported with suggested fixes"
  - "[ ] Phase 3: instructions cross-referenced against package.json and current source to detect deprecated packages, removed scripts and dead patterns"
  - "[ ] Phase 4: rules with paths: frontmatter checked for at least one matching file, orphaned rules and coverage gaps listed"
  - "[ ] Phase 5: agent-memory files checked for dead references, contradictions with CLAUDE.md, size and session-specific entries"
  - "[ ] Phase 6: rot score computed with the size penalty and interpreted into a severity band"
  - "[ ] Phase 6: prioritized P0-P4 remediation plan generated"
  - "[ ] Auto-fix applied only to P0 items and only when fix_automatically is true"
  - "[ ] Halted rather than auto-fixing when the change set would touch more than 10 files"
  - "[ ] AEXOS-managed sections of CLAUDE.md left untouched"
---
...
| CLAUDE.md has AEXOS-managed sections | WARN -- do not modify managed sections |
