...
Saida: |
  ...
  - rules: .claude/rules/ files with paths: frontmatter for frontend, api, testing and database contexts
  - hooks: Hook configuration under .claude/hooks/ registered in settings.json, where the project benefits
  - mcp_config: MCP servers recommended for the detected stack, delegated to the mcp-workflow task and documented in CLAUDE.md
  - agents: Project-specific subagent definitions in .claude/agents/, sized to project complexity
  - integration_result: YAML summary with files created, MCP and agents configured, and the smoke test verdicts
Checklist:
  - "[ ] Phase 1: project root scanned for every detection signal, package.json read, structure pattern and existing lint/format rules identified"
  - "[ ] Phase 2: CLAUDE.md generated with the seven required sections and verified under 200 lines, overflow moved to rules"
  - "[ ] Phase 3: .claude/settings.json created with deny rules for sensitive paths and allow rules for development operations"
  - "[ ] Phase 4: .claude/rules/ created with path-frontmatter rules matched to the project structure"
  - "[ ] Phase 5: beneficial hooks assessed, created under .claude/hooks/ and registered in settings.json"
  - "[ ] Phase 6: MCP servers recommended for the stack, delegated to mcp-workflow and documented in CLAUDE.md"
  - "[ ] Phase 7: subagents created in .claude/agents/ proportional to project complexity, via create-agent-definition"
  - "[ ] Phase 8: smoke test run — config files valid, CLAUDE.md sized and complete, rules load, MCP connects, Claude responds following the conventions"
  - "[ ] Existing CLAUDE.md with managed sections merged rather than overwritten"
  - "[ ] Existing .claude/settings.json with custom rules merged rather than overwritten"
---
...
- Use managed sections (`<!-- AEXOS-MANAGED-START -->`) for auto-updatable content
