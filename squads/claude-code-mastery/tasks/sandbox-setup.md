...
Saida: |
  ...
  - filesystem_policy: allowWrite, denyWrite and denyRead path lists using the documented path prefixes
  - test_results: Expected vs actual with PASS/FAIL for denied reads, allowed writes, denied writes and domain filtering
  - excluded_commands: The commands that bypass the sandbox, each with its justification
Checklist:
  - "[ ] Phase 1: platform detected and its sandbox capability confirmed against the technology table"
  - "[ ] Phase 1: write, read-only, invisible, network and out-of-sandbox command requirements surveyed"
  - "[ ] Phase 1: isolation level chosen from the standard / strict / airgapped table"
  - "[ ] Phase 2: sandbox block generated in settings.json with each flag set per the recommendation column"
  - "[ ] Phase 3: allowedDomains built from the project type, with the isolation level applied"
  - "[ ] Phase 4: allowWrite, denyWrite and denyRead configured, with the always-deny read list (.env, secrets, ~/.ssh, ~/.aws, ~/.kube) included"
  - "[ ] Phase 5: sandbox activity, network restrictions and filesystem boundaries tested, with results documented in the test table"
  - "[ ] Sandbox not disabled without explicit user confirmation and documented justification"
  - "[ ] allowAllUnixSockets and allowUnsandboxedCommands left false for team and production environments"
  - "[ ] No home directory in the write-allowed paths and no wildcard domain in allowedDomains"
---
...
2. Survey project requirements:
   ...
   - Which directories should be read-only? (.aexos-core/, config files)
...
|-------|---------------|--------------|
| standard | src/, tests/, docs/, tmp/ | node_modules/, .git/, .aexos-core/core/ |
