...
Saida: |
  ...
  - roi_ranking: ROI score per candidate, ranked descending
  - rollout_plan: Phase A immediate, Phase B short-term and Phase C on-demand, each entry with config snippet, credentials and verify command
  - excluded_mcps: Servers rejected, each with its exclusion reason
Checklist:
  - "[ ] Phase 1: project examined across stack, external dependencies, workflow, documentation, testing and data needs, producing a needs matrix with frequencies"
  - "[ ] Phase 2: each need matched to candidate official and community MCP servers with a fit score"
  - "[ ] Phase 3: context cost estimated per server — tool registration, per-call cost, startup latency, memory — and totalled against the budget guidelines"
  - "[ ] Phase 3: any server registering more than 20 tools flagged as context-heavy"
  - "[ ] Phase 4: ROI computed from frequency, time saved, context cost and setup effort, and candidates ranked"
  - "[ ] Phase 5: phased rollout produced with config snippet, required credentials, verification command and budget impact per server"
  - "[ ] No MCP recommended that needs credentials the user has not agreed to provide"
  - "[ ] Stated budget level not exceeded without explicit user approval"
  - "[ ] Experimental or abandoned MCPs flagged with their maturity risk"
  - "[ ] Nothing installed or configured — the task delivered a plan only"
---
