...
Saida: |
  ...
  - git_conventions_section: Commit format, branch naming and PR rules extracted from history, under 10 lines
  - architecture_and_key_files: Key architecture decisions and critical files, combined under 30 lines
  - managed_sections: Frequently changing content wrapped in MANAGED-START/END markers
  - rules_extracted_to: .claude/rules/ files holding sections that were too large to keep inline
  - claude_md_engineer_result: YAML summary with line counts per section and the validation verdicts
Checklist:
  - "[ ] Phase 1: tech stack detected and the 5-10 project-specific patterns Claude does not already know identified"
  - "[ ] Phase 2: 5-10 representative source files analysed and only non-default standards documented"
  - "[ ] Phase 3: real testing commands and test-file conventions extracted from project config"
  - "[ ] Phase 4: commit message format, branch naming and PR rules derived from git history"
  - "[ ] Phase 5: key architecture decisions and critical files listed with their purposes"
  - "[ ] Phase 6: every line tested against \"would removing this cause a mistake?\", paragraphs converted to bullets or tables, sections over 30 lines moved to rules files"
  - "[ ] Phase 7: frequently changing sections wrapped in managed markers, stable sections left as plain markdown"
  - "[ ] Phase 8: the eight-item validation checklist run — line count, actionability, section size, path and command references, duplication, no tutorials, marker format"
  - "[ ] Phase 8: sample interactions run (new component, add test, commit) and missing instructions added"
  - "[ ] Final file confirmed under 200 lines with no filler content"
---
