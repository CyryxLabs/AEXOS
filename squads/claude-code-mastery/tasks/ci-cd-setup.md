...
Saida: |
  ...
  - secrets_docs: The secrets that must exist (ANTHROPIC_API_KEY) and how they are referenced
  - output_parsing: Format choice per pattern plus the parsing snippet and error-case handling
  - safety_config: max_turns, timeout_minutes, max diff lines, concurrency group and cost monitoring
  - ci_cd_setup_result: YAML summary with platform, pattern, files created, safety limits and estimated cost per run
Checklist:
  - "[ ] Phase 1: integration pattern chosen from the pattern table, with its prompt and expected output format defined"
  - "[ ] Phase 2: headless command composed per pattern and tested locally before entering CI"
  - "[ ] Phase 2: .claude/ci-system-prompt.md created for CI context"
  - "[ ] Phase 3: workflow file created with trigger events, timeout-minutes and a permissions block"
  - "[ ] Phase 4: ANTHROPIC_API_KEY stored as a repository secret and passed via env — never logged, never a command argument"
  - "[ ] Phase 5: output format selected per pattern, parsing script written when stream-json is used, and empty/timeout/rate-limit cases handled"
  - "[ ] Phase 6: --max-turns set on every claude -p invocation"
  - "[ ] Phase 6: diff-size guard and concurrency group configured, and cost monitoring set up"
  - "[ ] Pipeline verified to run successfully in headless mode"
---
