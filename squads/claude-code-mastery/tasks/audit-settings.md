...
Saida: |
  ...
  - security_gaps: Missing sensitive-path deny rules, each with its severity
  - findings: Severity-ranked table (CRITICAL / HIGH / MEDIUM / LOW / INFO) with layer and fix
  - optimization_opportunities: Redundant and duplicated rules that can be consolidated
Checklist:
  - "[ ] Phase 1: every accessible settings layer located, parsed and its permissions, MCP, hooks, sandbox and mtime extracted"
  - "[ ] Phase 2: rule, mode, array-merge and hook conflicts between layers identified, with deny-wins resolution noted"
  - "[ ] Phase 3: required sensitive-path deny rules validated (.env, secrets, *.pem, *.key, rm -rf, pipe-to-shell)"
  - "[ ] Phase 3: AEXOS-specific deny rules checked when .aexos-core/ exists"
  - "[ ] Phase 4: effective permission mode determined and assessed for appropriateness, including enterprise lockdown flags"
  - "[ ] Phase 5: every MCP server verified for command/URL, env-var references instead of hardcoded values, and a matching permission rule"
  - "[ ] Phase 6: audit report generated with severity-ranked findings"
  - "[ ] No settings file modified — the audit stayed read-only"
  - "[ ] Secret values reported as present only, never displayed"
  - "[ ] Audit not reported clean while critical deny rules are missing, and bypassPermissions never recommended as a fix"
---
...
**AEXOS-specific deny rules (if .aexos-core/ exists):**

| Pattern | Protects | Severity if Missing |
|---------|----------|---------------------|
| `Edit(.aexos-core/core/**)` | L1 Framework Core | HIGH |
| `Edit(.aexos-core/constitution.md)` | Constitution | HIGH |
| `Edit(bin/aexos.js)` | CLI entry point | MEDIUM |
