...
Saida: |
  ...
  - lifecycle_plan: The create, work, merge and cleanup flow with triggers, steps, timing and monitoring
  - stale_detection: The rules that mark a worktree stale, plus the prune and remove commands
  - merge_sequence: Dependency-ordered merge order, the per-merge commands and the conflict-resolution approach
  - worktree_strategy_result: YAML summary with the isolation verdict, worktrees, merge order and cleanup flags
Checklist:
  - "[ ] Phase 1: agent assignments analysed and a file-ownership matrix built to find files with multiple writers"
  - "[ ] Phase 1: shared repo documented and the remaining phases skipped when no write conflicts exist"
  - "[ ] Phase 2: base branch chosen and a {task-id}/{agent-name} branch planned per agent with its owned files"
  - "[ ] Phase 2: branch names verified not to collide with existing branches"
  - "[ ] Phase 3: a worktree created per agent as a sibling directory, verified with git worktree list, and dependencies installed"
  - "[ ] Phase 3: each agent configured to use its worktree as working directory"
  - "[ ] Phase 4: lifecycle documented across create, work, merge and cleanup, with stale-worktree detection and the cleanup commands"
  - "[ ] Phase 5: merges executed in dependency order, conflicts resolved, and the full test suite run on the merged result"
  - "[ ] Phase 5: worktrees removed, merged branches deleted and git worktree prune run"
  - "[ ] Halted rather than proceeding on uncommitted changes, missing commits, insufficient disk space or git older than 2.5"
---
