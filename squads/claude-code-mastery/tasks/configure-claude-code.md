...
Saida: |
  ...
  - permission_mode: The selected defaultMode with its rationale and the settings-hierarchy explanation
  - mcp_config: mcpServers entries for the chosen servers, credentials as placeholders, plus MCP permission rules
  - env_var_recommendations: Recommended environment variables with values and purpose, documented outside the JSON
  - configuration_summary: The markdown report with rule counts and the *audit-settings verification step
Checklist:
  - "[ ] Phase 1: technology markers, sensitive file patterns and safe development operations scanned and reported to the user"
  - "[ ] Phase 2: settings.json generated deny-first, with allow rules populated from the detected stack"
  - "[ ] Phase 3: permission mode selected from the security level table, presented with rationale and open to user override"
  - "[ ] Phase 4: MCP servers chosen with the user and configured with placeholder credentials and matching MCP permission rules"
  - "[ ] Phase 5: environment variable recommendations documented separately, since JSON has no comments"
  - "[ ] No settings.json produced without deny rules"
  - "[ ] No API key, token or credential hardcoded in any settings file"
  - "[ ] bypassPermissions not set as default unless explicitly requested and acknowledged"
  - "[ ] No destructive operation added to the allow list"
  - "[ ] Diff between old and new configuration shown to the user before merging"
---
...
1. Scan the project for technology markers:
   ...
   - AEXOS: Check for .aexos-core/ directory
