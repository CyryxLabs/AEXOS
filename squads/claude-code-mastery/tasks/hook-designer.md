...
Saida: |
  ...
  - io_contract: The input schema Claude Code supplies and the output the hook must return
  - hook_script: The implementation under .claude/hooks/, with error handling and timeout protection
  - registration_snippet: The settings.json hooks entry registering the hook with its timeout
  - test_plan: At least three scenarios — normal, edge and error — with inputs and expected outputs
  - performance_notes: Expected execution time, configured timeout and the failure mode
Checklist:
  - "[ ] Phase 1: need categorized (security, automation, quality, observability, context) and validated as hook-suitable, with an alternative recommended if not"
  - "[ ] Phase 2: transport and event chosen from the category table with the rationale stated"
  - "[ ] Phase 3: primary event, guard conditions, timeout and error behaviour determined"
  - "[ ] Phase 4: input contract, processing logic, output contract, performance requirements and any state handling designed"
  - "[ ] Phase 5: hook script written with error handling, registered in settings.json, and tested against sample input and edge cases"
  - "[ ] Phase 6: hook verified inside a real session — fired on the event, no interference with other hooks, completes well within timeout"
  - "[ ] No hook designed that blocks all tool use without an escape hatch"
  - "[ ] No hook sends data to external services without user consent"
  - "[ ] Timeout kept at or below 30 seconds, and stop error behaviour reserved for security hooks"
  - "[ ] Hook observes and gates only — it does not modify source code"
---
