...
Saida: |
  ...
  - rule_category: The category assigned from the category table, e.g. API, component, test, database, security
  - glob_match_report: How many real files each pattern matches, with warnings for zero or overly broad matches
  - activation_instructions: How the user can confirm the rule loads, plus a suggested test prompt
  - conflict_report: Any contradiction with CLAUDE.md, with the recommended resolution
Checklist:
  - "[ ] Phase 1: rule need elicited and categorized, or rules suggested from the scanned project structure"
  - "[ ] Phase 2: file path resolved to .claude/rules/{rule_name}.md and paths: frontmatter generated for conditional rules"
  - "[ ] Phase 2: frontmatter omitted entirely for always-on rules"
  - "[ ] Phase 3: rule body written with imperative, specific and verifiable instructions, using bullets and code examples"
  - "[ ] Phase 4: frontmatter YAML validated — delimiters, array form, quoted globs, no trailing whitespace or tabs"
  - "[ ] Phase 4: glob patterns tested against the project, warning on zero matches or overly broad matches"
  - "[ ] Phase 5: activation behaviour explained to the user with a concrete test prompt"
  - "[ ] Phase 5: any conflict with CLAUDE.md flagged with a recommended resolution"
  - "[ ] Rule file kept under 100 lines, split into focused rules if longer"
  - "[ ] No secret, API key or credential written into the rule file, and no large always-on rule created for path-specific content"
---
