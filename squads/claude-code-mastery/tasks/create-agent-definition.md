...
Saida: |
  ...
  - agent_type: general-purpose, explore or plan, with the rationale for the choice
  - instruction_body: Role, Process, Rules (ALWAYS/NEVER) and Output Format sections
  - model_choice: The selected model tier with its justification from the decision matrix
  - test_results: 2-3 representative prompts with pass/fail status and the ready flag
Checklist:
  - "[ ] Phase 1: primary responsibility written in one sentence, with 3-5 in-scope tasks, 2-3 anti-scope items and the required context"
  - "[ ] Phase 2: subagent type matched to purpose, or general-purpose chosen with explicit allowed-tools restrictions, and the rationale recorded"
  - "[ ] Phase 3: .claude/agents/{name}.md created from the template with complete frontmatter"
  - "[ ] Phase 4: Role, Process, Rules and Output Format sections written to be specific and bounded"
  - "[ ] Phase 5: model selected via the decision matrix and set in frontmatter, with per-call cost considered"
  - "[ ] Phase 6: agent invoked with a representative prompt and verified to respect its tools, instructions, output format and scope"
  - "[ ] Phase 6: 2-3 distinct test prompts run to cover edge cases"
  - "[ ] Agent purpose confirmed narrow enough to be a single agent rather than several"
  - "[ ] Output format defined before the file was created and required tools confirmed to exist"
  - "[ ] Agent name confirmed not to conflict with an existing agent, and no agent shipped with failing tests"
---
